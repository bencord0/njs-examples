...
// - client_id: From process.env.PATREON_CLIENT_ID
...
function login(r) {
    ...
    let patreon_domain = r.variables.patreon_domain;
    let url = `https://${patreon_domain}/oauth2/authorize`;
...
// - client_id: From process.env.PATREON_CLIENT_ID
// - client_secret: From process.env.PATREON_CLIENT_SECRET
// - code: The authorization code we received from the OIDC Authorization Endpoint
// - redirect_uri: The public URL to our callback endpoint
//
...
async function callback(r) {
    ...
    let patreon_domain = r.variables.patreon_domain;
    let url = `https://${patreon_domain}/api/oauth2/token`;
...
async function authenticate(r) {
    ...
    let url = `https://${patreon_domain}/api/oauth2/api/current_user`;
    ...
    r.variables.auth_user = profile.data.attributes.full_name;
    r.variables.auth_email = profile.data.attributes.email;
    r.variables.auth_unique_id = profile.data.id;
